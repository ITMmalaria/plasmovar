...
workflow PLASMOVAR {
    ...
    if (!skip_alignment) {
        ...
        ch_fastq_pe = ch_fastq_sorted.pe
            .map { meta, fq -> [meta.id, [meta.read_num, fq], meta] }   // [s-LANE_L00#-FC_####, [1, s_R1.fastq.gz], [meta]]
            .groupTuple().view()                                               // [s-LANE_L00#-FC_####,, [[1, s_R1.fastq.gz], [2, s_R2.fastq.gz]], [[meta_R1], [meta_R2]]
        ...
        // Final combined fastq channel that stores both PE and SE files
